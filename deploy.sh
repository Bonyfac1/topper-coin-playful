#!/usr/bin/env bash
# Deploy the playful Topper Coin site to Forpsi web hosting via FTP.
#
...
# never appears in process listings. --fail returns non-zero on FTP errors so
# `set -e` catches them.
for entry in "${UPLOADS[@]}"; do
  local_file="${entry%%:*}"
  remote_path="${entry#*:}"
